pipeline {
  ...
  stages {
      ...
      steps {
        script {
          ...
          withAWS(credentials: 'AWS_CREDENTIALS') {
            // Create EKS cluster
            sh 'eksctl create cluster --name tinkuk-ekscluster --region ap-south-1 --version 1.31 --nodegroup-name linux-nodes --node-type t2.micro --nodes 2'
          }
        }
      }
    }
    ...
    stage('Build Node JS Docker Image') {
      steps {
        script {
          sh 'docker build -t kushakumar/nodesss-app-1.0 .'
        }
      }
    }
    ...
    stage('Deploy Docker Image to DockerHub') {
      steps {
        script {
          ...
          sh 'docker push kushakumar/nodesss-app-1.0'
        }
      }   
    }
    ...
    stage('Deploying Node App to Kubernetes') {
      steps {
        script {
          sh ('aws eks update-kubeconfig --name tinkuk-ekscluster --region ap-south-1')
          sh "kubectl get ns"
